<template>
    <div class="entity-form">
        ...
        <form @submit.prevent="submit">
            <div class="form-group"><label for="name">{{ t("admin.collections.name") }}</label><input id="name" v-model="form.name" type="text" /></div>
            <div class="form-group"><label for="slug">Slug</label><input id="slug" v-model="form.slug" type="text" /></div>
            <div class="form-group"><label for="description">{{ t("admin.products.fields.description") }}</label><textarea id="description" v-model="form.description" rows="4"></textarea></div>
            <div class="form-grid">
                <div class="form-group"><label for="sort_order">{{ t("admin.categories.sortOrder") }}</label><input id="sort_order" v-model.number="form.sort_order" type="number" min="0" /></div>
                <div class="form-group"><label for="image_url">{{ t("admin.products.fields.imageUrl") }}</label><input id="image_url" v-model="form.image_url" type="text" /></div>
            </div>
            <div class="form-group"><label for="image">{{ t("admin.products.fields.image") }}</label><input id="image" type="file" accept="image/*" @change="handleFileChange" /></div>
